...
def _submit_launches(
    ...
    no_watch: bool = False, tty: bool = False, policy_overrides: list[str] | None = None,
) -> int:
    ...
    for it in launches:
        name = it["key"]
        rid, status, detail = core.submit(it, profile, dry_run, repo_root, policy_overrides)
...
def multinode(
    ...
    profile: Optional[str] = _PROFILE_OPT,
    policy: Optional[str] = typer.Option(None, "--policy", help="usage_policy_name to assign to every submitted run"),
    policy_id: Optional[str] = typer.Option(None, "--policy-id", help="usage_policy_id (UUID); mutually exclusive with --policy"),
    ...
    pol_overrides, perr = core.policy_override(policy, policy_id)
    if perr:
        console.print(f"[red]{perr}[/]")
        raise typer.Exit(2)
    ...
    if print_only:
        for it in launches:
            ...
            print("  " + shlex.join(core.air_cmd(it, profile, dry_run, repo_root, pol_overrides)))
        raise typer.Exit(0)

    raise typer.Exit(_submit_launches(
        launches, tier=tier, profile=profile, repo_root=repo_root,
        dry_run=dry_run, sequential=sequential, no_poll=no_poll,
        no_watch=no_watch, tty=tty, policy_overrides=pol_overrides,
    ))
